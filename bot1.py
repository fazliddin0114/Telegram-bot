...
async def react_to_channel_post(post: types.Message):
    ...
    try:
        ...
        logging.info(f"✅ {channel_id} kanaliga 1 reaksiya qo'yildi: {reaction}")
        ...
        if "not enough rights" in error_msg:
            channels_db[channel_id]['active'] = False
            logging.warning(f"⚠️ Kanalda reaksiya huquqi yo'q: {channel_id}")
            
        elif "too many" in error_msg or "reactions_too_many" in error_msg:
            # 5 daqiqalik tanaffus
            channels_db[channel_id]['active'] = False
            logging.warning(f"⚠️ Reaksiya limiti tushirildi. Kanal 5 daqiqaga bloklandi: {channel_id}")
            ...
            logging.warning(f"⚠️ Kanalda reaksiyalar o'chirilgan")
